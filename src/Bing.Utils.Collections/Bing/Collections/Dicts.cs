...
public static class Dicts
{
    ...
    /// <exception cref="ArgumentNullException"></exception>
    public static void AddValueIfNotExist<TKey, TValue>(Dictionary<TKey, TValue> dictionary, TKey key, Func<TKey, TValue> valueCalculator)
    {
        if (valueCalculator is null)
            throw new ArgumentNullException(nameof(valueCalculator));
        if (dictionary.ContainsKey(key))
            return;
        dictionary[key] = valueCalculator.Invoke(key);
    }
